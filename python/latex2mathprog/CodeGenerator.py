...
class CodeGenerator:
    ...
    def _getCodeConstraint(self, constraint):
        if isinstance(constraint, Constraint):
            ...
            return SUBJECT_TO+SPACE+"C" + str(self.constraintNumber) + SPACE + constraint.generateCode(self)
    ...
    def _processDomain(self, declaration, name, domain, dependencies_vec, sub_indices_vec, minVal, maxVal, stmtIndex):
        result = EMPTY_STRING
    ...
    def _posModel(self):
        res = BREAKLINE+SOLVE+END_STATEMENT+BREAKLINE+BREAKLINE+BREAKLINE
        res += self._declareDataSetsAndParams()
        res += END+END_STATEMENT
    ...
    def generateCode_Constraint(self, node):
        ...
        res += node.constraintExpression.generateCode(self) + END_STATEMENT

        return res

    def generateCode_ConstraintExpression2(self, node):
        return node.linearExpression1.generateCode(self) + COMMA+SPACE + node.op + SPACE + node.linearExpression2.generateCode(self)

    def generateCode_ConstraintExpression3(self, node):
        return node.numericExpression1.generateCode(self) + COMMA+SPACE + node.op + SPACE + node.linearExpression.generateCode(self) + COMMA+SPACE + node.op + SPACE + node.numericExpression2.generateCode(self)
    ...
    def generateCode_ConditionalLinearExpression(self, node):
        res = IF+SPACE + node.logicalExpression.generateCode(self) + SPACE+THEN+SPACE + node.linearExpression1.generateCode(self)
    ...
    def generateCode_SymbolicExpressionWithFunction(self, node):
        ...
        if node.function == SymbolicExpressionWithFunction.SUBSTR:
            res += node.symbolicExpression.generateCode(self) + COMMA + node.numericExpression1.generateCode(self)
            if node.numericExpression2 != None:
                res += COMMA + node.numericExpression2.generateCode(self)
        
        elif node.function == SymbolicExpressionWithFunction.TIME2STR:
            res += node.numericExpression1.generateCode(self) + COMMA + node.symbolicExpression.generateCode(self)
    ...
    def generateCode_IteratedSetExpression(self, node):
        return SETOF+SPACE+BEGIN_SET + node.indexingExpression.generateCode(self) + END_SET+SPACE + node.integrand.generateCode(self)
